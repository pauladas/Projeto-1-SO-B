...
static unsigned long mensagemCharInput_size = 0; 
static char       mensagemCharInput[MENSAGEM_MAX_SIZE];                   /* Armazena a mensagem em CARACTERES lida do arquivo do modulo. Obs: o tamanho eh "ilimitado", mas iremos considerar os primeiros 64*5(blocos) caracteres*/
...
static ssize_t dev_leitura(struct file *filep, char *buffer, size_t len, loff_t *offset)
{
   	int ret=0;
    	if(strlen(buffer) == 0) {
        	...
        	ret=copy_to_user(buffer,"cryptohashlixious",sizeof("cryptohashlixious"));
        	ret=sizeof("cryptohashlixious");
    	}
    	return ret;
}
 ...
 */
static ssize_t dev_escrita(struct file *file, const char *buffer, size_t len, loff_t *offset){
   	mensagemCharInput_size = len;
    	if (mensagemCharInput_size > MENSAGEM_MAX_SIZE)
        	mensagemCharInput_size = MENSAGEM_MAX_SIZE;

    	if (copy_from_user(mensagemCharInput, buffer, mensagemCharInput_size))
        	return -EFAULT;
	
    	mensagemCharInput[mensagemCharInput_size] = '\0';
    	pr_info("%s", mensagemCharInput);
    	return mensagemCharInput_size;
}
